...
fn run_python(
    ...
    let cli_path = python_folder.join("product_catalog_cli.py");

    if !cli_path.is_file() {
        return Err(format!(
            "ไม่พบ Product Catalog Engine ที่ {}",
            cli_path.display(),
        ));
    }

    let mut command = Command::new(get_python_path(&project_path));
    command
...
fn get_project_path() -> Result<PathBuf, String> {
    let cargo_folder = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    cargo_folder
        .parent()
        .map(Path::to_path_buf)
        .ok_or_else(|| "ไม่พบโฟลเดอร์โปรเจกต์".to_string())
}

fn get_python_path(project_path: &Path) -> PathBuf {
    #[cfg(target_os = "windows")]
